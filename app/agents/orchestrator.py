...
from app.agents.state import AgentState, PropertyMatch
...
class PropertySalesAgent:
    ...
    def _classify_intent(self, state: AgentState) -> AgentState:
        ...
        logger.info(f"Classified intent: {found_intent} (raw: {intent[:50]})")
    ...
    def _generate_response(self, state: AgentState) -> AgentState:
        ...
        elif intent == "searching_properties" or intent == "gathering_preferences":
            properties = state.get("recommended_properties", [])
            prefs = state.get("preferences", {})
...
Confirm the booking enthusiastically. Let them know a representative will contact them soon."""
    ...
    async def process_stream(
        ...
        try:
            # Run through graph except final response generation
...
property_agent = PropertySalesAgent()
