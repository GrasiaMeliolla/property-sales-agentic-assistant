...
export default function ChatWidget() {
  ...
  return (
    <>
      ...
      {/* Chat Panel */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-96 max-h-[70vh] h-[500px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-silver-200">
          {/* Header */}
          <div className="bg-gradient-to-r from-primary-700 to-primary-600 text-white px-4 py-3 flex items-center gap-3 shrink-0">
          ...
          {/* Input */}
          <div className="p-3 bg-white border-t border-silver-200 relative z-10 shrink-0">
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 px-4 py-2.5 bg-silver-100 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:bg-white transition-all text-silver-900 placeholder-silver-500"
                disabled={isLoading || !conversationId}
              />
              <button
                onClick={handleSend}
                disabled={!input.trim() || isLoading || !conversationId}
                className="w-10 h-10 bg-primary-600 hover:bg-primary-700 disabled:bg-silver-300 text-white rounded-full flex items-center justify-center transition-colors shrink-0"
              >
